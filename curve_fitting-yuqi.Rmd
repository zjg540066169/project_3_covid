...
data = data_reg %>% filter(country_region == "China")
max_case = max(data$region_confirmed)
curve_fitting(data, tol = 0.01,vec_start = c(max_case,0.2,30))
```
...
curve_fitting2 = function(data,tol = 1e-10, max = 2000,vec_start){
  ...
  while(i<= max){
    ...
    if(abs(sse_p-sse_n)<=tol &(sse_n<sse_0)){
      break
    }
  }
 return(res)                     
...
max_case = max(data$region_confirmed)
result = curve_fitting2(data, tol = 1e-10,vec_start = c(max_case,2,40))
result
...
data = data_all %>% filter(country_region == "China")
max_case = max(data$region_confirmed)
result = curve_fitting3(data, tol = 0.01,vec_start = c(10000,10,30))
result
