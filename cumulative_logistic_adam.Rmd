...
cumu_logit_adam = function(t, y, parameters = NULL, epoch = NULL, learning_rate = 1e-3, momentum_decay = 0.9, ada_decay = 0.999, tol = 1e-4, print_loss = FALSE, region_name = NULL){
  ...
  while (TRUE) {
    ...
    a_gradient = sum(loss_gradient * (1 / (1 + exp(-b * (t - c)))))
    b_gradient = sum(loss_gradient * (a * exp(-b * (t - c)) * (t - c) / ((1 + exp(-b * (t - c))) ^ 2))) 
    c_gradient = sum(loss_gradient * (-a * exp(-b * (t - c)) * b / ((1 + exp(-b * (t - c))) ^ 2))) 
    
    # momentum udate
    a_momentum = momentum_decay * a_momentum + (1 - momentum_decay) * a_gradient
    # adaGrad update
    a_ada = ada_decay * a_ada + (1 - ada_decay) * a_gradient^2
    # parameter update
    ...
    # adaGrad update
    b_ada = ada_decay * b_ada + (1 - ada_decay) * b_gradient^2
    # parameter update
    ...
    # adaGrad update
    c_ada = ada_decay * c_ada + (1 - ada_decay) * c_gradient^2
    # parameter update
...
out <- foreach(i = 1:4, .combine = c) %dopar% {
...
b = result[[2]]
c_ = result[[3]]
plot(seq(1, 100), a / (1 + exp(-b * (seq(1, 100) - c_))))
```
